    ...
    C: Ciphersuite,
{
    ...
    /// Verifies that a key package is valid aka no zero or base point values
    /// and the min signers is greater than 1
    pub fn is_valid(&self) -> bool {
        self.identifier.is_valid()
            && self.signing_share.is_valid()
            && self.verifying_share.is_valid()
            && self.verifying_key.is_valid()
            && self.min_signers > 1
    }
}
    ...
    C: Ciphersuite,
{
    ...
    /// Verifies that a public key package is valid aka no zero or base point values
    pub fn is_valid(&self) -> bool {
        self.verifying_shares.iter().all(|(_, vs)| vs.is_valid()) && self.verifying_key.is_valid()
    }
}
