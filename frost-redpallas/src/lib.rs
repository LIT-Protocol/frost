...
/// The context string for FROST(Pallas, BLAKE2b-512).
/// TODO: this hasn't been formalized yet, so it's subject to change.
const CONTEXT_STRING: &str = "FROST(Pallas, BLAKE2b-512)";
...
impl Field for PallasScalarField {
    ...
    fn deserialize(buf: &Self::Serialization) -> Result<Self::Scalar, FieldError> {
        let repr = (*buf).into();
        match Scalar::from_repr(repr).into() {
            Some(s) => Ok(s),
...
impl Group for PallasGroup {
    ...
    fn deserialize(buf: &Self::Serialization) -> Result<Self::Element, GroupError> {
        let repr = (*buf).into();
        let point = Self::Element::from_bytes(&repr);
...
impl Ciphersuite for PallasBlake2b512 {
    ...
    fn H4(m: &[u8]) -> Self::HashOutput {
        hash_to_array(&[CONTEXT_STRING.as_bytes(), b"FROST_RedPallasM", m])
    }

    /// H5 for FROST(Pallas, BLAKE2b-512)
    fn H5(m: &[u8]) -> Self::HashOutput {
        hash_to_array(&[CONTEXT_STRING.as_bytes(), b"FROST_RedPallasC", m])
    }

    /// HDKG for FROST(Pallas, BLAKE2b-512)
    fn HDKG(m: &[u8]) -> Option<<<Self::Group as Group>::Field as Field>::Scalar> {
        Some(hash_to_scalar(
            (CONTEXT_STRING.to_owned() + "FROST_RedPallasD").as_bytes(),
            m,
        ))
    }
