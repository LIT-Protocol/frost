...
//  after `cargo fmt`)
use crate::{Ciphersuite, CryptoRng, Identifier, KScalar, RngCore, frost};
...
) -> Result<BTreeMap<Identifier, KScalar>, Error> {
...
///
/// Returns a scalar
pub fn repair_share_step_2(deltas_j: &[KScalar]) -> KScalar {
    frost::keys::repairable::repair_share_step_2::<Secp256K1Taproot>(deltas_j)
}
...
mod tests {
    ...
    fn check_repair_share_step_1() {
        ...
        frost_core::tests::repairable::check_repair_share_step_1::<Secp256K1Taproot, _>(rng);
    }

    #[test]
    fn check_repair_share_step_2() {
        frost_core::tests::repairable::check_repair_share_step_2::<Secp256K1Taproot>(&REPAIR_SHARE);
    }

    #[test]
    fn check_repair_share_step_3() {
        let rng = thread_rng();
        frost_core::tests::repairable::check_repair_share_step_3::<Secp256K1Taproot, _>(
            rng,
            &REPAIR_SHARE,
        );
    }
